...
struct _loop_impl
{
                    ...
                    std::array<T, n>& i)
    {
        for (i[m - 1] = from[m - 1]; i[m - 1] < to[m - 1]; ++i[m - 1]) {
            _loop_impl<n, m - 1, T>::run(from, to, func, i);
        }
    }
};

template<u8 n, typename T>
struct _loop_impl<n, 0, T>
{
    template<class F>
...
struct loop
{
    ...
    {
        std::array<T, n> i;
        _loop_impl<n, n, T>::run(from, to, func, i);
    }
};

// loop_with_counter
template<u8 n, u8 m, typename T, typename C = u64, typename I = u64>
struct _loop_with_counter_impl
{
    template<class F>
    static void run(const std::array<T, n>& from,
                    const std::array<T, n>& to,
                    ...
                    std::array<T, n>& i)
    {
        for (i[m - 1] = from[m - 1]; i[m - 1] < to[m - 1];
            ...
            _loop_with_counter_impl<n, m - 1, T, C, I>::run(
...
struct _loop_with_counter_impl<n, 0, T, C, I>
{
    template<class F>
    static void run(const std::array<T, n>&,
                    const std::array<T, n>&,
...
struct loop_with_counter
{
    ...
    {
        std::array<T, n> i;
        _loop_with_counter_impl<n, n, T, C, I>::run(
            from, to, counter, increment, func, i);
    }
};
}
